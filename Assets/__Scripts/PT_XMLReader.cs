...
public class PT_XMLReader {
	...
	*/
	...
	public void Parse(string textToParse) {
		...
		Parse(textToParse, xml);
	}
	
	// This function will parse a possible series of tags
	void Parse(string textToParse, PT_XMLHashtable tagData) {
		textToParse = textToParse.Trim();
        // Replace any tabs with spaces
        textToParse = textToParse.Replace('\t',' ');
		while(textToParse.Length > 0) {  //text is run to the end
			textToParse = ParseTag(textToParse, tagData);
			textToParse = textToParse.Trim();
		}
	}
	
	// This function parses a single tag and calls Parse() if it encounters subtags
	string ParseTag(string textToParse, PT_XMLHashtable tagData) {
		// search for "<"
		int tagOpenBraceIndex = textToParse.IndexOf("<");
		int end, end1, end2, tagCloseBraceIndex;
		if (tagOpenBraceIndex == -1) {
			// It's possible that this is just a string (e.g. <someTagTheStringIsInside>string</someTagTheStringIsInside>)
			tagCloseBraceIndex = textToParse.IndexOf(">"); // This closes a standard tag; look for the closing tag
			if (tagCloseBraceIndex == -1) {
				// In that case, we just need to add an @ key/value to the hashtable
				textToParse = textToParse.Trim(); // I think this is redundant
				//eH["@"] = eS;
				tagData.text = textToParse;
			}
			return(""); // We're done with this tag
			// end of Parse func.
		}
		// Ignore this if it is just an XML header (e.g. <?xml version="1.0"?>)
		if (textToParse[tagOpenBraceIndex+1] == '?') {
			// search for the closing tag of this header
			int ndx2 = textToParse.IndexOf("?>");
			string header = textToParse.Substring(tagOpenBraceIndex, ndx2-tagOpenBraceIndex+2);
			//eH["@XML_Header"] = header;
			tagData.header = header;
			return(textToParse.Substring(ndx2+2));
		}
		// Ignore this if it is an XML comment (e.g. <!-- Comment text -->)
		if (textToParse[tagOpenBraceIndex+1] == '!') {
			// search for the closing tag of this header
			int ndx2 = textToParse.IndexOf("-->");
			string comment = textToParse.Substring(tagOpenBraceIndex, ndx2-tagOpenBraceIndex+3);
			if (SHOW_COMMENTS) Debug.Log("XMl Comment: "+comment);
			//eH["@XML_Header"] = header;
			return(textToParse.Substring(ndx2+3));
		}
		...
		end1 = textToParse.IndexOf(" ", tagOpenBraceIndex);	// This means that we'll have attributes
		end2 = textToParse.IndexOf("/", tagOpenBraceIndex);	// Immediately closes the tag, 
		tagCloseBraceIndex = textToParse.IndexOf(">", tagOpenBraceIndex);	// This closes a standard tag; look for the closing tag
		if (end1 == -1) end1 = int.MaxValue;
		if (end2 == -1) end2 = int.MaxValue;
		if (tagCloseBraceIndex == -1) tagCloseBraceIndex = int.MaxValue;
		
		
		end = Mathf.Min(end1, end2, tagCloseBraceIndex);
		string tag = textToParse.Substring(tagOpenBraceIndex+1, end-tagOpenBraceIndex-1);
		
		// search for this tag in eH. If it's not there, make it
		if (!tagData.ContainsKey(tag)) {
			tagData[tag] = new PT_XMLHashList();
		}
		// Create a hashtable to contain this tag's information
		PT_XMLHashList arrL = tagData[tag] as PT_XMLHashList;
		...
		string atts = "";
		if (end1 < tagCloseBraceIndex) {
			try {
				atts = textToParse.Substring(end1, tagCloseBraceIndex-end1);
			}
		...
		bool singleLine = (end2 == tagCloseBraceIndex-1);// ? true : false;
		if (!singleLine) { // This is a multiline tag (e.g. <tag> ....  </tag>)
			// find the closing tag
			int close = textToParse.IndexOf("</"+tag+">");
// TODO: Should this do something more if there is no closing tag?
			...
			subs = textToParse.Substring(tagCloseBraceIndex+1, close-tagCloseBraceIndex-1);
			leftoverString = textToParse.Substring( textToParse.IndexOf(">",close)+1 );
		} else {
			leftoverString = textToParse.Substring(tagCloseBraceIndex+1);
		}
